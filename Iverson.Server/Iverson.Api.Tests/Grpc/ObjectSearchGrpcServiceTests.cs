...
public class ObjectSearchGrpcServiceTests
{
    ...
    // ── SimilarRetrievalVector selection (temporary — see VectorRankingOptions) ─────────────────

    // Doc.Body is dual-annotated (chunked + embedded), so centroidPossible is true and
    // SimilarRetrievalVector="centroid" takes effect: SearchSimilar retrieves candidates by the
    // centroid named vector and fetches the head vector as the fusion (secondary) signal.
    [Fact]
    public async Task SearchSimilar_ChunkedPropertyWithCentroidSetting_RetrievesByCentroid_FusionFetchesHeadVector()
    {
        await _registry.RegisterAsync(DualAnnotatedSchema());
        ...
        var results = new List<VectorSearchResult>
        {
            new(1, 0.90, new Dictionary<string, string> { ["body"] = "A" }),
        };
        _vector.SearchNamedAsync("docs_test-tenant", "body_centroid", queryVector, Arg.Any<ulong>(), Arg.Any<Filter>())
               .Returns(results.AsReadOnly());
        _vector.RetrieveNamedVectorAsync(Arg.Any<string>(), Arg.Any<IReadOnlyList<ulong>>(), Arg.Any<string>())
               .Returns((IReadOnlyDictionary<ulong, float[]>)new Dictionary<ulong, float[]>());
        ...
        var sut = new ObjectSearchGrpcService(
            ...
            Options.Create(new VectorRankingOptions
                { LambdaSimilar = 0.70, LambdaChunks = 0.70, SimilarRetrievalVector = "centroid" }),
            Options.Create(new DecayOptions()));

        var (writer, written) = MakeStream<SearchResponse>();
        await sut.SearchSimilar(
            new SearchSimilarRequest { TypeName = "Doc", Property = "Body", Query = "q", TopK = 1 },
            writer, TestServerCallContext.Create());

        // The stub only matches SearchNamedAsync(..., "body_centroid", ...) — a non-empty result
        // proves that is the name the code actually retrieved by.
        written.Should().HaveCount(1);
        await _vector.Received().RetrieveNamedVectorAsync(
            Arg.Any<string>(), Arg.Any<IReadOnlyList<ulong>>(), "body_vector");
    }

    // Same schema, but SimilarRetrievalVector left at its default "head" — today's behaviour.
    // Uses the shared _sut, which pins no SimilarRetrievalVector and so keeps the "head" default.
    [Fact]
    public async Task SearchSimilar_ChunkedPropertyWithHeadSetting_RetrievesByHeadVector_FusionFetchesCentroid()
    {
        await _registry.RegisterAsync(DualAnnotatedSchema());
        ...
        // The stub only matches SearchNamedAsync(..., "body_vector", ...) — a non-empty result
        // proves that is the name the code actually retrieved by.
        written.Should().HaveCount(1);
        await _vector.Received().RetrieveNamedVectorAsync(
            Arg.Any<string>(), Arg.Any<IReadOnlyList<ulong>>(), "body_centroid");
    }

    // Article.Title is embedding-only — centroidPossible is false regardless of the setting, so
    // SimilarRetrievalVector="centroid" must have no effect: the property is not chunked, so
    // there is no centroid named vector to retrieve by in the first place.
    [Fact]
    public async Task SearchSimilar_NotChunkedProperty_RetrievesByHeadVector_RegardlessOfSetting()
    {
        await _registry.RegisterAsync(SchemaFixtures.ArticleSchema());
        ...
        var results = new List<VectorSearchResult>
        {
            new(1, 0.90, new Dictionary<string, string> { ["title"] = "A" }),
        };
        _vector.SearchNamedAsync("articles_test-tenant", "title_vector", queryVector, Arg.Any<ulong>(), Arg.Any<Filter>())
               .Returns(results.AsReadOnly());
        ...
        // The stub only matches SearchNamedAsync(..., "title_vector", ...) — a non-empty result
        // proves that is the name the code actually retrieved by, "centroid" setting notwithstanding.
        written.Should().HaveCount(1);
        await _vector.DidNotReceive().SearchNamedAsync(
            Arg.Any<string>(), "title_centroid", Arg.Any<float[]>(), Arg.Any<ulong>(), Arg.Any<Filter?>());
    }

    // The diversity vector: under centroid retrieval at λ < 1, a genuine second retrieve fetches
    // the centroid by name and feeds IT (not the head vector already held for fusion) to the
    // diversifier. Distinct payloads per name make this observable in the OUTPUT: the head vector
    // is uniform across candidates (so it cannot itself explain any reordering), while the
    // centroid payload marks B as a near-duplicate of A and C as dissimilar. Only a diversifier
    // that actually received the centroid payload suppresses B in favour of C.
    [Fact]
    public async Task SearchSimilar_CentroidRetrieval_LambdaBelowOne_DiversifierReceivesCentroidVector()
    {
        await _registry.RegisterAsync(DualAnnotatedSchema());
        ...
        {
            new(1, 1.00, new Dictionary<string, string> { ["body"] = "A" }),
            new(2, 0.95, new Dictionary<string, string> { ["body"] = "B-near-duplicate" }),
            new(3, 0.60, new Dictionary<string, string> { ["body"] = "C-dissimilar" }),
        };
        _vector.SearchNamedAsync("docs_test-tenant", "body_centroid", queryVector, Arg.Any<ulong>(), Arg.Any<Filter>())
               .Returns(results.AsReadOnly());

        // Fusion (secondary) fetch — the head vector, uniform across all three candidates.
        ...
        // Diversity fetch — the centroid, which actually discriminates B from C.
        _vector.RetrieveNamedVectorAsync(
                   Arg.Any<string>(), Arg.Any<IReadOnlyList<ulong>>(), "body_centroid")
               .Returns((IReadOnlyDictionary<ulong, float[]>)new Dictionary<ulong, float[]>
               {
        ...
        // With the head vector uniform, fused-score order tracks base score exactly: A, B, C.
        // MMR(B) = 0.7*0.975 - 0.3*1.0 = 0.3825; MMR(C) = 0.7*0.80 = 0.56 — C wins, so the pick
        // order is [A, C]. Had the diversifier instead been fed the (uniform) head vector, B and
        // C would look equally similar to A and B — the higher fused score — would win instead.
        written.Should().HaveCount(2);
        written[0].Data.Fields["Body"].StringValue.Should().Be("A");
        written[1].Data.Fields["Body"].StringValue.Should().Be("C-dissimilar");
    }

    // At λ = 1.00 the fused-score ordering IS the output (MMR reduces to Take(topK)), so no
    // output-level assertion can tell what the diversifier was fed — B and C's fused scores are
    // untouched by whichever vector reaches DiversityVector. A capturing fake is required instead:
    // it asserts directly on what DiversifyCandidate.DiversityVector held, and separately proves
    // no second retrieve was issued (the whole point of the λ = 1.00 short-circuit).
    [Fact]
    public async Task SearchSimilar_CentroidRetrieval_LambdaOne_DiversifierReceivesHeadVector_NoSecondRetrieve()
    {
        await _registry.RegisterAsync(DualAnnotatedSchema());
        ...
        var headVector = OrthogonalUnitVector();
        _vector.RetrieveNamedVectorAsync(
                   Arg.Any<string>(), Arg.Any<IReadOnlyList<ulong>>(), "body_vector")
               .Returns((IReadOnlyDictionary<ulong, float[]>)new Dictionary<ulong, float[]> { [1] = headVector });

        // Substitute.For<IResultDiversifier>() auto-substitutes the IReadOnlyList<RerankedResult>
        // return with an empty list, so the SearchSimilar foreach that consumes it writes nothing
        // to the response stream — expected here; the assertions below are on what this fake
        // CAPTURED, not on stream output.
        IReadOnlyList<DiversifyCandidate>? captured = null;
        var fakeDiversifier = Substitute.For<IResultDiversifier>();
        fakeDiversifier.Diversify(
            Arg.Do<IReadOnlyList<DiversifyCandidate>>(c => captured = c), Arg.Any<int>(), Arg.Any<double>());
        ...
        var sut = new ObjectSearchGrpcService(
            ...
            fakeDiversifier,
            Options.Create(new VectorRankingOptions
                { LambdaSimilar = 1.00, LambdaChunks = 0.70, SimilarRetrievalVector = "centroid" }),
            Options.Create(new DecayOptions()));
        ...
        captured.Should().NotBeNull();
        captured!.Single().DiversityVector.Should().Equal(headVector);
