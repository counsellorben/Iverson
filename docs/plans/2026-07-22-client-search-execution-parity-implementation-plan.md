...
Add `acting_user_token: str | None = None` keyword param. Change the branch condition at `core.py:370` from `if credentials is not None:` to `if credentials is not None or acting_user_token is not None:`. Inside that branch, `call_creds`'s construction (currently unconditional — `provider = _CachedTokenProvider(credentials)`, `call_creds = grpc.metadata_call_credentials(_BearerTokenAuthPlugin(provider))`) must become conditional on `credentials is not None` independently of the outer branch — building `_CachedTokenProvider(None)` when only `acting_user_token` is set would crash with `AttributeError` on the first RPC call, since the auth plugin re-reads `self._credentials.client_id` on every call, not just once (CIR round 1 finding, `2026-07-22-client-search-execution-parity-implementation-plan-critical-review-1.md` §2.1). Build a list of whichever call-credentials are actually configured (`call_creds` only if `credentials is not None`; a new `grpc.metadata_call_credentials(_ActingUserAuthPlugin(acting_user_token))` only if `acting_user_token is not None`) and pass whichever are present into the existing `grpc.composite_channel_credentials(...)` call (`core.py:378`).
