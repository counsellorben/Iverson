...
- [ ] **Step 4: Create or drop collections.** `--drop` deletes and recreates both collections empty **and deletes the progress file with them** — a `--drop` that left it behind would make the next `--resume` skip every document and report success over an empty collection. Object collection carries `body_vector` and `body_centroid`; chunks carries `body_vector`; all 768, Cosine.
