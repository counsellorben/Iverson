...
| 18 | Command / consumer impact | `IdentityScenario.DeniedStatusCode` is referenced at exactly 6 sites in `IdentityScenarioTests.cs`, not 2: `:62` (the shared `JudgeHappy` fixture's own `denied` default — most of the file's tests call `JudgeHappy()` with no override and inherit this), `:116` and `:146` (direct `IdentityScenario.Judge`/`JudgeTenantDerivation` calls, neither of which inspects the enforcement/response-shape assertion), `:424` (the diagnostic-detail test, which checks only `.Detail`, not `.Passed`), and `:614`/`:652` (bypass `JudgeHappy`, grade cell/detail wiring, not the enforcement predicate's polarity). `:62`'s default must become `DeniedStep(null)` (the new "happy" default is an accepted write); `:116`/`:146` can safely become `DeniedStep(null)` too, since neither asserts on the enforcement cell; `:424` should stay a literal `7` to preserve the fixture's original intent (a driver reporting an actual denial code alongside descriptive status/message text); `:614`/`:652` become a literal `7` per the same reasoning as before | Read all 6 sites in full this session (`:56-62`, `:111-123`, `:142-149`, `:410-431`, `:600-634`, `:648-656`); confirmed only `:424`'s test's own smoke-test sibling (`Judge_ReadBackAndDenialBothAsExpected_AllAssertionsPass`, which calls `JudgeHappy()` with no override and asserts `.OnlyContain(a => a.Passed)`) is load-bearing on `:62`'s specific value — the other four sites' host tests don't inspect the enforcement/response-shape assertion at all |
...
- [ ] **Step 2: `ObjectMappingGrpcService.cs` — same narrowing, `MappingResponse` shape**
  ...
  Replace `:384-389` (the "before" text is extended two lines earlier than the changed code itself, to `_relationValidator.ValidateAndNormalizeRelations(...)` and `var payloadJson = ...`, specifically because the 4 lines that actually change are byte-identical to code inside `Post` at `:322-325` — a plain string-replace on the shorter block matches both methods; these two preceding lines exist only in `Update`):
  ```csharp
  ...
  var outboxRowId = await _outboxWriter.UpsertAndEnqueueOutboxAsync(
      ...
      tenantId: decision.TenantValue);
  ```
  with (the two preceding lines unchanged):
  ```csharp
  _relationValidator.ValidateAndNormalizeRelations(request.Payload, schema);
...
- [ ] **Step 2: `IdentityScenario.cs` — guarded predicate, rewritten details, repointed crefs, Backstop rewrite**
  ...
  ```
  The trailing `assertions[^1] = ...` statement is unchanged from the current file (`:482-486`) — it sits inside the cited `:456-486` range but was omitted from both fences in an earlier draft of this plan; it must survive, since two existing tests (`Judge_Idn003EnforcementDetail_CarriesTheReportedStatusNameAndMessage`, `Judge_DriverReportedNoStatusNameOrMessage_DetailSaysSoRatherThanThrowing`) assert on the `Detail` string it produces.
...
- [ ] **Step 3: `IdentityScenarioTests.cs` — invert, rename, repoint (12 sites)**
  ...
  - `:62` (`JudgeHappy`'s own default parameter) — change `denied ?? DeniedStep(IdentityScenario.DeniedStatusCode)` to `denied ?? DeniedStep(null)`: the new "happy" default is an accepted write (this is the concrete repointing the spec's item 2 called for and the plan's earlier draft never stated).
  - `:116` (`Judge_NoSeededKey_BackstopFailsAndCarriesNoRequirementId`) and `:146` (`Judge_NoReadStepAtAll_Idn002FailsRatherThanBeingSkipped`) — replace `DeniedStep(IdentityScenario.DeniedStatusCode)` with `DeniedStep(null)`. Neither test inspects the enforcement/response-shape assertion (one checks only the backstop assertion, the other only `Cited(assertions, "IVC-IDN-002")`).
  - `:424` (`Judge_Idn003EnforcementDetail_CarriesTheReportedStatusNameAndMessage`) — replace `statusCode = IdentityScenario.DeniedStatusCode` with `statusCode = 7`. This test checks only `.Detail`, not `.Passed`, but a literal `7` preserves the fixture's original intent (a driver reporting an actual denial code alongside descriptive status/message text).
