...
- [ ] **Step 3: New Helm subchart `charts/admin-ui`** — `Chart.yaml` (name/version), `values.yaml` (image repo/tag/pullPolicy, resources, replicas), `templates/deployment.yaml` (env vars `OIDC_CLIENT_ID` via the same `secretKeyRef` pattern `charts/api/templates/deployment.yaml` already uses against `{{ .Release.Name }}-authentik-human-oidc-client`/`client-id`; `OIDC_AUTHORITY`/`API_BASE_URL` as plain values sourced from `.Values.oidcAuthority`/`.Values.apiBaseUrl` (subchart-scoped — the parent's `adminUi:` alias key is stripped before this template sees it, same as `charts/api/templates/deployment.yaml` reads `.Values.image.repository`, never `.Values.api.image.repository`), populated per environment in Step 5), `templates/service.yaml` (ClusterIP :8080), `templates/ingress.yaml` (`host: {{ .Values.global.ingressHost }}`, `path: /admin` Prefix).

- [ ] **Step 4: Register the new subchart** in the parent `Chart.yaml`'s `dependencies:` list, alongside `api`/`worker`/`authentik`/etc. Set `alias: adminUi` on this dependency entry — Helm requires the parent's values-override key to exactly match the dependency's `name:` or an explicit `alias:`, and `admin-ui` (matching the folder/chart name) is hyphenated while every values-file edit in this plan uses the camelCase `adminUi:` key; without the alias, Helm silently ignores those overrides.

- [ ] **Step 5: Add `adminUi:` blocks** to all five values files, matching the existing per-subchart structure (image/resources/replicas), plus `oidcAuthority`/`apiBaseUrl` fields (kind's `oidcAuthority` matching Task 2's Authentik ingress host, e.g. `http://authentik.iverson.local/application/o/iverson-api/`; `apiBaseUrl` matching each environment's existing `api.ingress.host` — `http://iverson.local` for kind, `https://iverson.example.com` for aws/azure/gcp, per those files' own TLS-termination convention), plus the cloud files' CORS/TLS placeholders for this ingress per the spec's Cloud Wiring section.
