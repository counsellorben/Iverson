...
| 15 | Consumer impact | `log_connections=on` does not threaten journald retention | journald holds 3.9 G over 15.2 days ≈ 263 MB/day. At ~34 conn/min ≈ 49k/day × 2 lines × ~120 B ≈ **12 MB/day, +4.5%** → retention 15.2 → ~14.5 days, against a 1–2 day crash interval |
| 16 | Command | `docker`, `date`, `mkdir`, `flock` all present | `command -v` for each returned a path |
...
awk 'END{print NR" lines"}' "$L"; grep -c '^' "$L"  # equal ⇒ no multi-line records
head -2 /tmp/sigpipe-smoke/containers.csv        # a timestamped container map row (§4.2)
```

- [ ] **Step 5: Clean up the smoke output and leave the stack as found.**
```bash
rm -rf /tmp/sigpipe-smoke
docker stop iverson-postgres
```
...
docker start iverson-postgres
until docker exec iverson-postgres pg_isready -U iverson >/dev/null 2>&1; do sleep 1; done
docker exec iverson-postgres psql -U iverson -c "
