...
| P24 | Code validity | The BCa interval step 4 pins is constructible here without new machinery | scipy 1.18.1 from the pinned `python-libs` exposes `bootstrap(data, statistic, *, method='BCa', …)` and performs the jackknife internally; over a cluster resample of the judged population it returned finite bounds with no degenerate-data warning. `report.py` supplies no bootstrap and none is needed |
| P25 | Code validity | The jackknife unit "the 672 queries" is correct on **both** populations, not just the all-multi arm it was written from | the dump, the 92,758 and the 10,874 each span exactly 672 distinct queries |
| P26 | Code validity | Reading B's judged strata also survive the cluster bootstrap — P22's degeneracy evidence covered only reading A, whose strata are equal-sized by construction | reading B's smallest judged stratum (360 pairs) holds 81 relevant pairs across **47 distinct queries**; stripping a class needs all 47 absent from a 672-draw resample (≈ 1e-20), so `mannwhitneyu`'s empty-input path is unreachable under either reading |
| P27 | Code validity | The seed keyword step 4 passes to `scipy.stats.bootstrap` is still accepted — P24 covered `method='BCa'` and the internal jackknife, not the seed | `random_state=` is present in scipy 1.18.1's signature and emits no `DeprecationWarning`; `rng=` is the modern spelling |
...
- [ ] **Step 3: Stratified AUC and advantage.** Stratify by `max_chunk` decile (`np.quantile` + `np.digitize`, P18), compute each candidate's AUC of relevant vs irrelevant within each stratum via `mannwhitneyu` (P17), pool weighted by stratum size, and report both the **raw AUC** and the **advantage over `count`** on the same pairs.
  ...
  **Decile edges are computed under both readings, and the per-population reading decides.** Reading A recomputes `np.quantile` within each label population; reading B computes the edges once on the 92,758 and reuses them for the judged population. **Reading A is the deciding computation** — it reproduces the spec's own recorded A20 (`AUC(count)` 0.4707), A22 (56 judged cells) and A25 (+0.0307), and keeps judged strata at 1087–1088 rather than 360–2,870. Reading B is computed and published in the verdict document as a sensitivity column carrying **raw AUC and advantage with BCa intervals, and `AUC(count)`, per population — but no permutation p**. The intervals are what make the column comparable against the same two CI clauses the GO rule tests; the p is omitted because reading B decides nothing and a Holm-adjusted p would make the column a complete shadow of the GO conjunction, inviting exactly the post-hoc reading the plan forbids. It is on the record, and it decides nothing. Both readings coincide on the all-multi population, which is the population B's edges come from.
...
- [ ] **Step 6: CLI and both populations.** Arguments for the dump, keymap, qrels and β = 0 run file. One pass computes both label populations — judged-only (10,874) and all-multi with unjudged scored irrelevant (92,758) — and applies the GO conjunction from spec §2.6, with Holm across the screen-1 survivors via `report.holm_adjust` and `report.HOLM_ALPHA`. Print every quantity spec §2.7 requires in the verdict document, **and for the deciding reading A the raw-AUC and advantage BCa intervals and the Holm-adjusted permutation `p_adj`, per candidate per population** — spec §2.6 requires these on the record as magnitudes and §2.7's list carries no interval or p at all — **plus the reading-B sensitivity figures of step 3 — raw AUC and advantage with their BCa intervals, and `AUC(count)`, per population — and the bootstrap's seed, replicate count and interval construction from step 4**, so Task 2 has one source to transcribe from. Step 4's requirement that the bootstrap parameters be restated in the verdict document is discharged here: this clause is their producer.
...
- [ ] **Step 2: Write the verdict document** at `docs/plans/2026-09-GATE-coverage-screen.md`, in the shape of `docs/plans/2026-09-GATE-chunk-coverage-phase2.md`. Spec §2.7 mandates all of: every candidate's degeneracy ρ; its raw AUC **and** advantage in both populations; **`AUC(count)` per `max_chunk` decile**; the **full ρ(τ) curve** for `n_within_tau`; and both control reproductions. Task 1 step 6 adds three more, none of which §2.7 carries: **reading A's raw-AUC and advantage BCa intervals and its Holm-adjusted `p_adj`** per candidate per population — the quantities step 3's anti-coverage classification actually reads, and the ones spec §2.6 requires on the record as magnitudes; the **reading-B decile sensitivity column**, published beside them; and the bootstrap's replicate count and interval construction, which are decision inputs.
