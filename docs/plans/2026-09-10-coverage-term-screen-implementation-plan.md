...
- [ ] **Step 6: CLI and both populations.** Arguments for the dump, keymap, qrels and β = 0 run file. One pass computes both label populations — judged-only (10,874) and all-multi with unjudged scored irrelevant (92,758) — and applies the GO conjunction from spec §2.6, with Holm across the screen-1 survivors via `report.holm_adjust` and `report.HOLM_ALPHA`. Print every quantity spec §2.7 requires in the verdict document, **plus the reading-B sensitivity figures of step 3 — raw AUC and advantage with their BCa intervals, and `AUC(count)`, per population — and the bootstrap's seed, replicate count and interval construction from step 4**, so Task 2 has one source to transcribe from. Step 4's requirement that the bootstrap parameters be restated in the verdict document is discharged here: this clause is their producer.
