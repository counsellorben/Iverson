...
| 17 | Code validity — **found during CIR round 1** | Singular (non-`repeated`) protobuf message-type fields in this repo's Google.Protobuf C# codegen default to `null` when unset, not a non-null-but-empty instance | `object_mapping.proto:76` (`TypeDescriptor root_type = 1;`, the same field category as the new `authorization` field), handled with `if (request.RootType is null) throw new RpcException(...)` at `ObjectMappingGrpcService.cs:49-50` |
| 18 | Code validity — **found during CIR round 1** | C# record-generated `Equals()` compares `IReadOnlyList<T>`-typed properties by reference (`List<T>` doesn't override `Equals`), not structurally — `AuthorizationRules`/`RowPermission`/`FieldPermission` (Task 1 Step 2) all have such properties, so `.Should().Be(...)` is unsafe for the Task 3 round-trip assertion; `.Should().BeEquivalentTo(...)` is the established fix in this test project | `Iverson.Api.Tests/Schema/SchemaBuilderTests.cs:68,130` (existing precedent for `.BeEquivalentTo(...)` on collection-bearing comparisons) |
...
```
Then, inside `BuildDescriptor`, map `typeDesc.Authorization` (a `ContractsAuthorizationRules?`) to the new `SchemaDescriptor.Authorization` property using a direct null check — `typeDesc.Authorization is null ? null : new SchemaAuthorizationRules(...)` — matching the exact pattern `RootType is null` already establishes for this same field category (`ObjectMappingGrpcService.cs:49-50`). Singular protobuf message fields default to `null` when unset, not a non-null-but-empty instance; project each `ContractsRowPermission`/`ContractsFieldPermission` into its `Schema*` counterpart only when `typeDesc.Authorization` is non-null.
...
```
This must go **after** `run()`'s existing `except Exception as e: log(...); return 1` clause closes — i.e. alongside `print(access_token)`, not inside the preceding `try:` block that `access_token = exchange_code_for_token(...)` is the last line of. Placed inside the `try:` block, any exception from `decode_jwt_claims` would be caught by that generic handler and reported as a hard script failure (exit 1, no token printed) — contradicting the "not a hard failure" intent below. Insert immediately before `print(access_token)`:
```python
