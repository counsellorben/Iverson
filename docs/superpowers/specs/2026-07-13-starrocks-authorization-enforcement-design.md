...
| 18 | `ValidateStepAndComputeOutput`'s metrics loop (Pipeline steps) has no existing validation of `MetricSpec.Expression`'s content — unlike every other Layer-1 construct, which each have a pre-existing `RequireColumn`/`ValidateWindow`/`ValidateDeriveExpr` hook | Read `StarRocksPipelineBuilder.cs` — the metrics loop calls `RequireColumn` only when `m.Field` is set; when `m.Expression` is set instead, nothing validates it before `EmitMetric` splices it into the generated SQL unchanged. (Resolved via CDR round 4 §2.1 — this construct needs a new token-parse check mirroring `ValidateDeriveExpr`, added directly to this loop; it cannot simply "extend" an existing pass the way the other constructs can.) |
...
- `Iverson.StarRocks.Tests`: `StarRocksQueryBuilder`/`StarRocksPipelineBuilder` unit tests for the new `authz` parameter — wrap-and-AND SQL shape for primary-type ownership, `ON`-clause-appended predicate for joined-type ownership (including a `LEFT JOIN` case proving row-preservation), `TrackAndValidate` rejecting a disallowed field/`all: true`-against-a-fresh-type reference with the new constraint present, an `Expression` token-parse rejecting a disallowed column, a filter-clause (`where`) reference to a disallowed column being rejected, a step joining `all: true` against a *prior step* silently succeeding (not rejected) regardless of restriction, a multi-step Pipeline (explicit multi-type `select` in an earlier step, pure implicit passthrough in the final step) whose response correctly retains the joined-type/derived column rather than having it stripped by the final masking pass, and a `PipelineStep.Metrics` entry referencing a disallowed column via `Expression` (instead of `Field`) being rejected the same way the top-level `Aggregate`/`GroupBy` bypass is.
