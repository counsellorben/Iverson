...
- `Iverson.StarRocks.Tests`: `StarRocksQueryBuilder`/`StarRocksPipelineBuilder` unit tests for the new `authz` parameter — wrap-and-AND SQL shape for primary-type ownership, `ON`-clause-appended predicate for joined-type ownership (including a `LEFT JOIN` case proving row-preservation), `TrackAndValidate` rejecting a disallowed field/`all: true` reference with the new constraint present, an `Expression` token-parse rejecting a disallowed column, and a filter-clause (`where`) reference to a disallowed column being rejected.
