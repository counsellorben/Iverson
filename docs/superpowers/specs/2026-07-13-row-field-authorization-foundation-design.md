...
4. Field-level (Read/Write only — Delete always returns `AllowedFields = null`, no field granularity for a whole-row delete): collect `FieldPermission` entries where the acting user's roles intersect `readable_roles`/`writable_roles` for the given action. If the schema declares no field-level rules at all → `AllowedFields = null` (unrestricted).
