...
`Article.cs:14` pins 512/64 explicitly and is left alone. The only server-side change is the
`SchemaBuilder` template fallback above, so that one FAIL verdict moves every producer of the default
window together; the consumer still reads each field's window from the registered schema, so
already-registered types keep their window until re-registered, and the spec's gate document says so.
The client standard does not state the default and needs no edit.
...
| gated (§3.4) | five client defaults (three sites in the Python client), `SchemaBuilder.cs:161-162`, `IngestContractTests`, regenerated contract |
