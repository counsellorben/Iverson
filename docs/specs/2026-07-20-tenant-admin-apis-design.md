...
**Ad-hoc `tenant_id` assignment is no longer supported.** Nothing in `docs/user-management-and-security.md`'s existing "Creating a human user and granting operator access" procedure is tenant-specific — it exists solely to onboard operators and is unaffected by this work. What changes is that manually editing a user's `attributes.tenant_id` in the Authentik admin UI, outside `CreateTenant`/`InviteUser`, is no longer a supported way to provision a tenant user: any `tenant_id` introduced that way has no `IversonTenants` registry row and is permanently denied by the fail-closed suspension check. `docs/user-management-and-security.md` should gain a note to this effect as part of this work, without altering its existing operator-onboarding steps.
...
| 16 | `ServerCallContext.GetHttpContext().User` is a valid, populated principal for any ASP.NET-Core-hosted gRPC service in production (only bare `TestServerCallContext`-based unit tests need a fallback, already solved by Part C's `TestServerCallContext.Create(user:)` overload) | `RegisterSchema`'s identical existing use of `context.GetHttpContext().User` (Part C); `TestServerCallContext.cs`'s `Create(user:)` overload (Part C, Task 4) |
