...
| 4 | With `RecencyBoost ≤ 10⁶`, popularity is finite for every parseable input with a non-negative count, including `SaturationPoint` at its extremes | IEEE-double mirror of `PopularityFor`: 0 non-finite of 216 cells over count, D, boost and S from the smallest positive double to `double.MaxValue`; negative counts reach −∞ at `effective = −S` (e.g. count −50, S 50, no bucket series so D = 0, at any boost), confirmed against the real `PopularityFor` and `ResultReranker` |
