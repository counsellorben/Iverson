...
3. `Iverson.ClientConformance/Requirements.cs` — loses the `IdnTenancyDerivedAndEnforced` const (a `Retired` row may not carry one) and gains two successor consts (see item 4 below); the three `Requirements.IdnTenancyDerivedAndEnforced` citations in `IdentityScenario.cs` and the `"IVC-IDN-003"` literals in `IdentityScenarioTests.cs` repoint to whichever successor now covers what each assertion grades; the `Count(...).Should().Be(3)` assertion is re-reasoned for the new requirement mix.
