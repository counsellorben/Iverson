...
1. `Iverson.ClientConformance/Scenarios/IdentityScenario.cs` — `DeniedStatusCode` is deleted rather than re-valued: `const int` cannot express "no status code" and C# forbids `const int?`. The enforcement assertion's predicate becomes `deniedStep is { Ok: true } && code is null` — **not** a bare `code is null`. `code` (`:458`) is computed as `deniedStep is { Ok: true } ? ReadStatusCode(deniedStep.Entity) : null`, so a bare inversion would grade green when the driver reported no denied step at all, when its attempt threw, or when it reported a non-numeric code — none of which is the acceptance `IVC-IDN-007` names. Keeping `deniedStep` in the predicate is what makes the Statement's "attempted … is answered" observable. The failure-detail strings (which currently name acceptance as the failure mode) are rewritten.
2. `Iverson.ClientConformance.Tests/IdentityScenarioTests.cs` — invert `Judge_WrongActingUsersUpdateWasDenied_Idn003EnforcementPasses` and `Judge_WrongActingUsersUpdateSucceeded_Idn003EnforcementFails`, repoint the shared `JudgeHappy` fixture's `denied` default at the acceptance shape, and rewrite `Judge_Idn003EnforcementDetail_…` (whose asserted detail strings are currently a denial's). Leave `Judge_WrongActingUsersUpdateFailedWithSomeOtherCode_…` unchanged — a code other than "none" still means something other than the swallow fired. `Judge_NoDeniedStepAtAll_Idn003EnforcementFailsRatherThanBeingSkipped` (`:394`), `Judge_DeniedStepItselfBroke_Idn003EnforcementFails` (`:403`) and `Judge_EmptyDocument_EveryIdnRequirementStillHasAFailingAssertion` (`:446`) keep asserting failure and need no polarity change — but only under the guarded predicate above; under a bare `code is null` all three would flip green. Additionally repoint every remaining `IdentityScenario.DeniedStatusCode` reference: `:62` (the `JudgeHappy` default) and `:424` (the detail test) as already stated, plus `:116` (`Judge_NoSeededKey_BackstopFailsAndCarriesNoRequirementId`), `:146` (`Judge_NoReadStepAtAll_Idn002FailsRatherThanBeingSkipped`), `:614` and `:652` (the two `GradeReads` wiring tests) — none of these four asserts on the enforcement assertion's own polarity, so each should pass the denial code as a literal `7` instead of the deleted constant.
3. `Iverson.ClientConformance/Requirements.cs` loses the `IdnTenancyDerivedAndEnforced` const (a `Retired` row may not carry one) and gains two successor consts (see item 4). **Every** reference to the deleted identifier repoints to whichever successor covers what the site grades: the three `Assertion` citations in `IdentityScenario.cs` (the response-shape successor at the enforcement-assertion site; the derivation successor at the two tenant-derivation sites), the three `Requirements.IdnTenancyDerivedAndEnforced` **symbol** uses in `IdentityScenarioTests.cs` (not just the `Count` assertion — two further live expressions inside `GradeReads_TheReadPhaseJudgement_ReachesTheCellCarryingItsIdnCitations` also reference the const by name and must repoint too, both to the derivation successor since that test grades the derivation cell), and the `"IVC-IDN-003"` string literals in the same file. The three `<see cref="IdnTenancyDerivedAndEnforced"/>` doc comments (in `IdentityScenario.cs` and `Requirements.cs`) must also repoint — dangling ones are compiler warnings, not errors, but they're dead links in the rationale the standard designates as authoritative. One of these three doc-comment sites — `IdentityScenario.cs:125-134`'s class doc comment — carries the same Backstop rationale paragraph as the standard's own Backstop section; it needs the identical rewrite described in item 4 below, not just its cref repointed. The `Count(...).Should().Be(3)` assertion splits into two concrete counts rather than being "re-reasoned": the three assertions it currently counts are two derivation observations and one enforcement observation, so it becomes an assertion of 2 for the derivation successor and a sibling assertion of 1 for the response-shape successor.
4. `docs/standards/iverson-client-standard.md` — per the standard's own Global Constraint 3, a row's Statement cell is immutable and cannot be reworded. Retire `IVC-IDN-003` instead (Status → `Retired`, Statement byte-unchanged) and author two Active successors, matching the document's own `IVC-IDN-004→005` and `IVC-REG-001→003` precedent for exactly this situation (a Statement that would otherwise become literally false):
   - A derivation-only successor (`IVC-IDN-006`) whose Statement carries only the still-true claim: the server derives a row's tenant from the acting-user identity rather than from the write payload.
   - A response-shape successor (`IVC-IDN-007`) whose Statement is written at the altitude the surviving assertion actually observes and no wider (per the resolved forced decision below): that a mapped update attempted by an acting user of another tenant is answered without a gRPC error status, the same as an accepted one. The harness observes the numeric status code only, never the response body, so a Statement about the response *envelope* would be a claim no assertion discharges — precisely the defect `IVC-IDN-004`'s own retirement (cited above) exists to prevent recurring. If envelope equality is ever wanted as a normative claim later, it needs an assertion that observes the body first — a five-language driver change, out of scope here.
   Add a Deferred row for the genuine enforcement gap this leaves: the server no longer signals cross-tenant denial on the wire at all, by design, so no client-observable assertion can discharge true enforcement — the only remaining evidence is the server's own audit entry (`reason=BlockedCrossTenantWrite`), which no conformance run reads. This mirrors an existing sibling Deferred entry's identical "no client can read the audit log" reasoning. Repoint the existing single Coverage row at `IVC-IDN-006`, narrowing its area name to the derivation claim that survives, and **add a second, separate Covered area for `IVC-IDN-007`** — one Coverage row per Active requirement, never one row shared by two, or the coverage gate's Mode 5 fails the unclaimed requirement (and a combined comma-separated citation would satisfy the gate mechanically but collapse two distinct claims behind one area name, which Mode 7's own rationale warns against). Rewrite the IDN **Backstop assertion** section (`iverson-client-standard.md:574-584`) rather than repointing its citation — its premise (that the negative leg is a denial that only means something while the seeded row exists) is removed by this fix: after it, *every* cross-tenant update takes `EnforceWriteAuthorization`'s create branch, and there is no denial assertion left. The backstop is now **more** load-bearing, not less: `IVC-IDN-007` is satisfied by the absence of an error status, which an unseeded fixture also produces, so "the write phase reported a row key for this language" is the only assertion left that separates a genuine swallowed cross-tenant update from a scenario that had nothing to update. State this explicitly, and narrow the closing sentence — the backstop is no longer strictly weaker than `IVC-IDN-007` (there is a state, no seeded row, where it is strictly stronger); it stays strictly weaker than `IVC-IDN-002` alone. Also update two existing Deferred rows this fix falsifies: the IDN-axis row whose "graded green" conclusion inverts once a header-less caller is graded red instead of green under the new assertion, and the ERR-axis row whose "and a tenant mismatch" clause and "`IVC-IDN-003` grades that code" cross-reference are both false now that the `TenantMismatch` branch is unreachable. Finally, update the document's own self-declared requirement count (a prose sentence stating "45 `Active` requirements") — this change is net +1 (one retired, two authored), so the count becomes 46; nothing in the coverage gate parses that sentence, so a stale count would not be caught in CI.

**Forced decision, resolved — the surviving assertion gets its own requirement, not an orphaned citation:** `IdentityScenario`'s "denied a write" assertion keeps running on every test (item 1 inverts it, it does not delete it), which conflicts with simply marking the whole area "Deferred" — this standard defines `Deferred` as "no assertion observes it," and one does, on every run. Resolved by authoring `IVC-IDN-007` above specifically to be what that surviving assertion cites: the *enforcement* claim goes Deferred (true — no assertion discharges cross-tenant write denial anymore), but the *response-shape* observation the harness actually still makes is owned by its own Statement rather than left uncited.
...
| 21 | The enforcement assertion's operand `code` (`IdentityScenario.cs:458`) is computed as `deniedStep is { Ok: true } ? ReadStatusCode(deniedStep.Entity) : null`, so a bare `code is null` predicate cannot distinguish "no denied step reported," "the driver's attempt threw," or "a malformed status code" from genuine acceptance — the predicate must guard on `deniedStep is { Ok: true }` as well | Read `IdentityScenario.cs:457-458` and `:640-646` (`ReadStatusCode`) directly at this HEAD |
| 22 | `IdentityScenario.DeniedStatusCode` is referenced by name at exactly 10 sites: 4 in `IdentityScenario.cs` (`:64,184,470,478`) and 6 in `IdentityScenarioTests.cs` (`:62,116,146,424,614,652`); `:614`/`:652` construct `DeniedStep(IdentityScenario.DeniedStatusCode)` inline rather than through the shared `JudgeHappy` fixture, so repointing `JudgeHappy`'s default does not reach them | `grep -n "DeniedStatusCode" Iverson.Server/Iverson.ClientConformance.Tests/IdentityScenarioTests.cs Iverson.Server/Iverson.ClientConformance/Scenarios/IdentityScenario.cs`, run against this HEAD |
| 23 | The IDN Backstop assertion's "strictly weaker than `IVC-IDN-002` and `IVC-IDN-003`/`007`" claim is false once the cross-tenant `Update` collision is narrowed: in the no-seeded-row state the backstop assertion fails while `IVC-IDN-007` passes, making the backstop strictly *stronger* there, not weaker | Hand-checked both grade states against `IdentityScenario.cs:125-134`'s and `iverson-client-standard.md:574-584`'s claim, and the existing `Judge_NoSeededKey_BackstopFailsAndCarriesNoRequirementId` test (`:112-123`) |
