...
Only the interactive, acting-user-driven request path should switch to `iverson_runtime` — background/system processes that legitimately need cross-tenant visibility must not be tenant-restricted. Checking actual consumers of the shared `IRecordStoreQueryExecutor`/`IRecordStoreTransactionRunner` interfaces (`PostgresRepository`) confirms a clean split:

- **Switches to `iverson_runtime`:** `EntityRepository`, whose methods are called directly from `ObjectRetrievalGrpcService` (reads), `ObjectMappingGrpcService` (Get and Delete's inline transaction), `ObjectPersistenceGrpcService` (the pre-write existence-check read that feeds `AuthorizationFieldMasking.EnforceWriteAuthorization`'s comparison logic), and `EntityRelationResolver` (relation-traversal reads) — confirmed these are the exact call sites by grepping every `entities.*`/`_entities.*` invocation across the gRPC layer.
- **Stays on plain `iverson`:** `DlqRepository`, `ReconciliationQueueRepository`, `SchemaRegistryRepository`, and `OutboxWriter`'s own outbox-table statement — confirmed these are the only other consumers, and all are cross-tenant system/background operations Part A never touched either.

`EntityRepository`'s five methods (`FetchByKeyAsync`, `FetchManyByKeysAsync`, `FetchByColumnAsync`, `FetchAllAsync`, `DeleteAsync`) take no tenant parameter today (confirmed by reading `EntityRepository.cs` directly) — each gains one, sourced from the already-computed `AuthorizationDecision.TenantValue` at each call site, mirroring exactly how Part A threaded `TenantColumn`/`TenantValue` through the codebase.

**Session-variable safety under connection pooling.** `PostgresRepository` pools connections via the Npgsql default (confirmed: plain `new NpgsqlConnection(connectionString)`, no pooling override). Only 1 of its 4 connection-acquisition points (`ExecuteInTransactionAsync`) currently runs inside an explicit transaction — the other 3 (`QueryAsync`, `ExecuteAsync`, `QuerySingleOrDefaultAsync`), covering all reads plus several writes, are single ad-hoc statements. Setting `app.tenant_id`/`SET LOCAL ROLE` safely on a pooled connection requires an explicit transaction, since only `SET LOCAL` (not plain `SET`) is guaranteed by Postgres itself to auto-revert at transaction end regardless of success or failure — so `QueryAsync`/`ExecuteAsync`/`QuerySingleOrDefaultAsync` gain explicit `BEGIN`/`COMMIT` wrapping around the tenant-scoped path specifically (not the unrestricted plumbing path, which is unaffected).
...
| Npgsql/Dapper can execute arbitrary raw SQL (including `SET`/DDL) with no special restriction | Read `PostgresSchemaManager.cs` directly — it already executes raw multi-keyword DDL (`CREATE TABLE`, `ALTER TABLE`) through the identical `conn.ExecuteAsync(sql)` path this design reuses | Confirmed by existing precedent in this codebase. |
