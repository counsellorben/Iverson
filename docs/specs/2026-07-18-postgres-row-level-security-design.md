...
`EntityRepository`'s five methods (`FetchByKeyAsync`, `FetchManyByKeysAsync`, `FetchByColumnAsync`, `FetchAllAsync`, `DeleteAsync`) take no tenant parameter today (confirmed by reading `EntityRepository.cs` directly) — each gains a **nullable** one (`string? tenantId`), sourced as described above for the 4 interactive consumers. The 3 background consumers (`ReconciliationService`, `IntelligenceStoreConsumer`, `EngagementStoreConsumer`) pass `null`: a `null` value means the method runs entirely under the plain `iverson` role, issuing no `SET LOCAL ROLE`/`set_config` at all.

`IOutboxWriter.UpsertAndEnqueueOutboxAsync(TableSchema, string, string, string)` has the same gap and the same fix: it gains the same nullable tenant parameter, sourced from `decision.TenantValue` at its two call sites (`ObjectMappingGrpcService`'s and `ObjectPersistenceGrpcService`'s `Post`/`Update`) — no ordering problem here, since `EnforceWriteAuthorization` has already succeeded by the time either service calls it.
...
**A real wrinkle this solves cleanly, made concrete:** `OutboxWriter.UpsertAndEnqueueOutboxAsync` writes to both the tenant-scoped entity table and the cross-tenant outbox table in one transaction (confirmed by reading `OutboxWriter.cs` directly — both statements run against the same `IDbTransactionContext`). `ObjectMappingGrpcService.Delete` does the same for its entity-delete + outbox-delete-enqueue pair (confirmed by reading `ObjectMappingGrpcService.cs` directly). Both go through `PostgresRepository.ExecuteInTransactionAsync` — the connection is already open and the transaction already started by the time either method's own statement runs, so the role/GUC switch happens *inside* the transaction callback via the same `IDbTransactionContext.ExecuteAsync` every other statement in this codebase already uses (an ordinary parameterized call, not a new primitive):

- `OutboxWriter.UpsertAndEnqueueOutboxAsync`'s callback issues `SET LOCAL ROLE iverson_runtime` then `SELECT set_config('app.tenant_id', @value, true)` via `tx.ExecuteAsync(...)` immediately before its entity-table upsert statement, then `RESET ROLE` via `tx.ExecuteAsync(...)` before its outbox-table insert statement.
- `ObjectMappingGrpcService.Delete`'s transaction callback does the same around its two calls: `DeleteAsync` issues `SET LOCAL ROLE`/`set_config` (via its own `tx` parameter) before its `DELETE` statement; `RESET ROLE` is issued before `EnqueueDeleteOutboxRowAsync`'s statement.

Because `SET (LOCAL) ROLE` can be issued more than once within a transaction, both statements land in one transaction, each under the correct privilege level, with no new interface or primitive beyond the `IDbTransactionContext.ExecuteAsync` this codebase already has.
...
| Exact set of consumers of `IEntityRepository` | Grepped every `entities.*`/`_entities.*` invocation, then re-grepped for all `IEntityRepository` injection sites repo-wide | **Revised to 7 consumers** (stated as 4 in this spec's first draft; corrected by critical-design-review round 2, §1): the 4 interactive gRPC-layer consumers (`ObjectRetrievalGrpcService`, `ObjectMappingGrpcService`, `ObjectPersistenceGrpcService`, `EntityRelationResolver`) plus 3 background consumers found by round 1's finding 2.3 (`ReconciliationService`, `IntelligenceStoreConsumer`, `EngagementStoreConsumer`). `AuthorizationFieldMasking` itself never calls `EntityRepository` — the gRPC service fetches the row first and passes the result to `AuthorizationFieldMasking.EnforceWriteAuthorization` for comparison. |
