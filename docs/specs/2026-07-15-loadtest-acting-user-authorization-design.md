...
- **`ActingUserTokenProvider`** — one instance per identity. Caches `{access_token, refresh_token, expires_at}` in memory; `GetTokenAsync()` refreshes via `grant_type=refresh_token` (no TOTP) when the cached token nears expiry, falling back to the full flow only on first use or if the refresh token is rejected. TOTP secret persisted to `~/.cache/iverson/acting-user-totp-secret-{target}-{username}.txt` (0600), where `{target}` is mapped from LoadTest's own `--target` value to the Python script's vocabulary (`"containers"` → `"compose"`, `"kind"` → `"kind"`) rather than used as-is — LoadTest's `--target` values (`Program.cs:17`: `"containers"`/`"kind"`) differ from the Python script's (`mint_acting_user_token.py:436`: `"compose"`/`"kind"`), and the cache must converge on the identical file for the same real environment so a user who already ran the documented `mint_acting_user_token.py --target compose` workflow (`docs/user-management-and-security.md:228`) — enrolling a server-side TOTP device for `iverson-acting-user-smoke-test` — doesn't hit Authentik's "already enrolled, secret unavailable" dead end when LoadTest mints natively.
...
- **`ReadPathScenario` is extended to also query `BenchmarkAuthor` and `BenchmarkTag`**, not just `BenchmarkArticle` (per CDR round 1 §3.1 — read-path must exercise all three entities' authorization rules, not leave two of them read-untested). Adds `GetMany`/`Get`-style key-based fetches for `BenchmarkAuthor`/`BenchmarkTag` IDs (sampled from Postgres the same way `BenchmarkArticle`'s IDs are today), each with an identity picked per call exactly like the existing `BenchmarkArticle` calls. `Search`/`Aggregate` coverage for these two entities is left to the implementation plan to size, since neither has `IversonSearchKey` fields today the way `BenchmarkArticle` does; any filter/aggregate profile added for them must explicitly decide whether to reference `Email`/`Category` (the restricted field) — referencing it exercises field-masking under `Search`/`Aggregate`, not just `Get`; avoiding it (as `BenchmarkArticle`'s existing profiles already do for `Body`) does not.
...
15. .NET's `HttpRequestMessage.Headers.Host` actually overrides the wire-level `Host` header sent by `HttpClient` (default `SocketsHttpHandler`) — confirmed empirically via a compiled probe (raw socket capture showed the overridden value sent exactly as set). This is what makes the Host-header issuer-mismatch workaround in §3 actually work in .NET, not just in Python.
...
- `ReadPathScenario`'s existing `BenchmarkArticle` filter/aggregate profiles were audited during CDR round 1 and confirmed to already exclude `Body` (the restricted field) — no overlap issue. The new `BenchmarkAuthor`/`BenchmarkTag` read coverage (§5) must make an explicit choice about referencing `Email`/`Category` in any filter/aggregate profile it adds — left to the implementation plan.
