...
`registrar.go:106`, `:227`, `:244`, `tags.go:250` and `coordinator.go:428` also switch on `Kind`, but over relation kinds only; they follow the `RelationKind` rename mechanically. `tags.go:250` is relation-vs-scalar routing plus tenant collection, not validation: its `switch fm.Kind` becomes a `fm.RelationKind != ""` test with the `default:` branch unchanged. Rewriting it against the scalar booleans would send every plain, untagged field into `meta.Relations`.
...
| G5 | **Five** sites read the scalar `Kind`, not one | `registrar.go:85-91`; `coordinator.go:125`, `:151`; `tags.go:246`; `sample/main.go:24`. **Five** further sites — `registrar.go:106`, `:227`, `:244`, `tags.go:250`, `coordinator.go:428` — switch over relation kinds only and follow the `RelationKind` rename. Both counts are complete, not merely accurate per listed site: `FieldMeta.Kind` is deleted, so any unlisted reader is a compile error. Verified by `grep -n '\.Kind\b'` across `Iverson.Clients/Go` excluding `generated/` |
| G6 | The five scalar `Kind*` constants are deletable | Referenced only in `ParseTag` (`tags.go:147-167`), the five scalar-read sites above, and the test suite. The relation sites read the relation constants, which survive |
...
| G12 | *(span-check gap, added round 1)* The Go assembly point can fail | `InspectType` is declared `(EntityMeta, error)` (`tags.go:213`) and already returns errors for a blank extract hint (`:242`) and the tenant-count rules (`:265-268`), so both the new tag parsing and §2's key-field check have an established path to raise |
