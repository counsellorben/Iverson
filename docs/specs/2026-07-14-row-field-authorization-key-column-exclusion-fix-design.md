...
No other file changes. Every downstream consumer of `AllowedFields` (11 call sites across `Iverson.Api` and `Iverson.StarRocks` — masking, reject-on-reference, and the write-path field rejection) derives its behavior from this one computation, so all of them are fixed by this single change with no call-site edits needed.
...
| 4 | All 11 `AllowedFields` call sites (`ObjectMappingGrpcService.cs` ×4, `ObjectRetrievalGrpcService.cs` ×2, `AuthorizationFieldMasking.cs` ×1, `ObjectSearchGrpcService.cs` ×5, `StarRocksQueryBuilder.IsFieldAllowed`, `StarRocksPipelineBuilder.ColumnsFor`) use a generic `allowedFields.Contains(...)`/`!allowedFields.Contains(...)` check with no special-casing that assumes the key column can legitimately be excluded | Read every call site listed; none branches differently based on whether the excluded field happens to be the key column |
