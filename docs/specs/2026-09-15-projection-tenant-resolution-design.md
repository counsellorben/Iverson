...
- `BuildSourceText(schema, row)` (`:145`) reads `row.Row`, and the tenant comes from `row.TenantId`. `row` has no other reader in the method.
...
| 19 | `EnrichmentConsumer` reads `row` only in `BuildSourceText` | `EnrichmentConsumer.cs:145-265`: the only `row` reference is `:145`; the later republish uses its own fetch (`:232`) |
