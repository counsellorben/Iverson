...
The write path (`ObjectPersistenceGrpcService.Post/Update`, `ObjectMappingGrpcService.Post/Update`) is enforced by `AuthorizationFieldMasking.EnforceWriteAuthorization`, not by a store read-predicate. It gains a tenant analogue of the ownership logic it already performs:
