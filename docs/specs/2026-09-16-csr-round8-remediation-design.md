...
Files: `Iverson.Server/Iverson.Api/Program.cs`, `Iverson.Server/Iverson.Api/Reconciliation/ReconciliationService.cs`, `docs/runbooks/chunk-collection-cleanup-2026-07.md`
...
// /v1/traces — data-plane listener only; its only legitimate route (the Ingress) already
// publishes it to service port 8080, so this costs nothing and closes the one gap the
// enumerated route list would otherwise leave on the wide-open health listener
app.MapPost("/v1/traces", ...).RequireAuthorization().RequireRateLimiting("traces").WithMetadata(new RequireListenerPort(8080));
```

The actual listener ports are read from `Kestrel:Endpoints:{Grpc,Http}:Url` at startup (matching the existing branch's `TryParseListenerPort` helper), not hardcoded — `Kestrel:Endpoints` is the binding that actually takes effect in this deployment. The middleware is deny-**when-marked**, not an allow-list: an endpoint with no `RequireListenerPort` metadata falls straight through to both listeners. Every endpoint `Program.cs` maps must therefore carry a marker (or be deliberately left unmarked, as the four data-plane gRPC services are — they're unreachable on `:8081` today since `AddGrpcWeb` is never called) — a future endpoint added without one silently inherits both.

**Protocol consequence for `/admin/*`:** `:8080` is bound `Protocols: Http2` (`appsettings.json:11-13`), so every HTTP/1.1 client of the three admin endpoints — including the platform's own documented manual-recovery procedure for DLQ exhaustion — must switch to HTTP/2 prior-knowledge. Confirmed empirically against the exact endpoint shape: plain HTTP/1.1 `curl` returns 400 on `:8080`; `curl --http2-prior-knowledge` returns 200 for the same request. Two alternatives were considered and rejected: widening the `Grpc` endpoint to `Http1AndHttp2` reopens an HTTP/1.1 surface on the one listener the Ingress can reach, weakening the boundary this fix draws; a third dedicated `Http1` admin listener adds chart/Service/NetworkPolicy surface for a `port-forward`/`exec`-only path. Instead, update the two places this procedure is documented: `docs/runbooks/chunk-collection-cleanup-2026-07.md`'s two `curl -X POST` commands (the port-forward form and the `exec` form) become `curl --http2-prior-knowledge -X POST ...`, and `ReconciliationService.cs:193-201`'s `LogCritical` operator message gains the same qualifier ("requires a manual in-cluster HTTP/2 POST (e.g. `curl --http2-prior-knowledge`) to /admin/reconcile/{Type}...").
...
| `:8080`'s `Http2`-only binding rejects a plain HTTP/1.1 client, so pinning `/admin/*` there breaks the documented manual-recovery procedure unless it switches to HTTP/2 prior-knowledge | Built and ran a real repro matching the exact `Kestrel:Endpoints` shape (Http2-only + Http1-only listeners, the spec's own `RequireListenerPort` middleware): plain HTTP/1.1 `curl -X POST` against the Http2-only listener returned 400; `curl --http2-prior-knowledge` returned 200; the same route on the Http1-only listener returned 404 (the marker working as designed) |
| `ObjectPersistenceGrpcService.RequireSchema` and `ObjectMappingGrpcService.RequireSchema` throw `FailedPrecondition` for an unregistered type but not for a foreign tenant's registered one — a real, pre-existing name-existence oracle this fix does not close | Read `ObjectPersistenceGrpcService.cs:165-168` and `ObjectMappingGrpcService.cs:509-511` directly — both are `registry.Get(typeName) ?? throw ... FailedPrecondition`, with no tenant check; contrasted against `ObjectRetrievalGrpcService.cs:30-38,82-89`, whose `EntityAccess.ForTenant(...)` argument makes RLS return `Found = false` identically for both cases, so only the two `RequireSchema` sites carry the oracle |
