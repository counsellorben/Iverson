...
**Key fields with incompatible flags are now rejected client-side.** All five clients now validate that key fields do not carry `search_key`, `large_field`, `embedding`, `chunk`, `metadata`, `summary`, `keywords`, or `extract_hint` — the exact set of flags that compose illegally with a key. (Key fields can still carry `description` alone.) Previously, such combinations were accepted but silently discarded during schema building: `SchemaBuilder.BuildDescriptor` collects every per-property declaration from a non-key loop (`SchemaBuilder.cs:53`).
