...
deployment with no entries behaves exactly as today. The default service (`options.Value.ModelId`) keeps
the global base URL.
...
- `--chunk-max-chars` / `--chunk-step` (defaults: the contract's `chunkWindow` values, 2,048 / 1,792),
  overriding `MAX_CHARS` / `STEP` for the run. `verify_contract`'s golden chunking cases (`:479`) keep
  running against the contract values, so the override cannot mask a chunker regression.
...
registration guard, all five clients, Helm (Phase 2).
...
| N0 (existing) | NFCorpus | nomic-embed-text (Ollama) | 768 | (as ingested 2026-08-27) | — (`nfcorpus-run-2026-08-27/runs/rerank-a0`) |
| N1 | NFCorpus | BAAI/bge-base-en-v1.5 (TEI) | 768 | matches N0 | ≈ 4.1 h |
...
N0's window must be read from its stats/`ingest.log` before N1 runs and matched with the override.
...
| S4 | Tests use a fake handler that can assert path/body and answer `/info` | `EmbeddingServiceTests.cs:14-31` |
