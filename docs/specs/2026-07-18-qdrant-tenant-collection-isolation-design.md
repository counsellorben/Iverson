...
**The lazy-create step is gated on the tenant value being non-null.** This is not optional polish — it's the mechanism that keeps the fail-closed sentinel path (above) actually fail-closed. The lazy-create step runs under the singleton `QdrantClient`'s **admin `api_key`** (the same credential `QdrantCollectionManager` always uses for collection management), not the per-call scoped JWT — so if lazy-create ran unconditionally, a null-tenant write would happily *create* `{CollectionName}__no-tenant-claim__` and then its rw-scoped JWT would succeed against a collection that now exists, silently writing the point instead of failing. Skipping lazy-create specifically when the tenant value is null means the sentinel collection is never created by this code path either, so the subsequent JWT-scoped upsert/delete call fails against Qdrant's own access check — for the identical reason the read path already fails closed (nothing ever creates a collection on the read side), rather than via a separate app-level null guard.
...
- **`IntelligenceStoreConsumer`**: tenant sourcing differs by operation, because the delete path has no live row left to re-fetch:
  - **Upsert** (create/update events): re-derive the tenant value from the authoritative Postgres row, the same pattern already used for owner re-derivation (`FetchAuthoritativeOwnerValueAsync`, `IntelligenceStoreConsumer.cs:233-246`) — `SchemaDescriptor.TenantColumn` is read the same way `Authorization.OwnerField` is read today.
  - **Delete** (`HandleDeleteAsync`): the row is already gone from Postgres by the time the delete event is consumed — `ObjectMappingGrpcService.Delete` deletes the row and publishes the event afterward, in that order (`ObjectMappingGrpcService.cs:221-240`) — so there is no authoritative row left to re-fetch; `FetchAuthoritativeOwnerValueAsync`'s `FetchByKeyAsync` would just return `null`. Instead, read the tenant value directly off `ev.PayloadJson`, the same pre-delete row snapshot (`rowJson`) the delete event already carries (`EntityEvent.PayloadJson`, populated at `ObjectMappingGrpcService.cs:239`) — captured server-side moments earlier in the same request, immediately after authorization succeeded, not client-supplied. This is a narrower trust decision than it might look: owner/tenant values are normally re-derived from Postgres rather than trusted from the event specifically because that value gets *persisted* into Qdrant's stored payload and relied on by every future `ApplyOwnership`/`ApplyTenant` read-time filter, so a forged event value would corrupt authorization going forward (`IntelligenceStoreConsumer.cs:91-93`, "CSR #7"). The delete path's use of this value is transient — it only selects which collection to route a one-time delete to, and is never persisted or reused afterward — so that specific risk doesn't transfer here.
  - Both operations: ensure the tenant's collection(s) exist (lazy create, described above) — **skipped when the tenant value is null**, per the fail-closed carve-out above. Mint a **read-write** (`readOnly: false`) JWT, wrap the upsert/delete calls.
...
- **Pre-Part-A legacy rows with a null tenant value.** If a row exists in Postgres under a now-tenant-scoped schema but predates Part A's tenant stamping (so its tenant column is genuinely null), `IntelligenceStoreConsumer`'s **upsert-path** tenant re-derivation would find null for that row — triggering the fail-closed sentinel behavior (the lazy-create skip, above), so such a row simply doesn't get its vector written until it's re-saved through the tenant-stamping write path, consistent with Part A's hard-cutover posture generally. Moot in practice today (no live tenant data exists yet, per direct confirmation from the user). Not applicable to the delete path, which sources its tenant value from the event's own pre-delete snapshot rather than a Postgres re-fetch (see "Call-site wiring" above).
...
| Every real (non-legacy) row has a non-null tenant value by the time `IntelligenceStoreConsumer` processes it | Read `AuthorizationFieldMasking.cs:48-49` (tenant stamped on every create where `decision.TenantColumn is not null`) plus the already-verified B1 invariant that `TenantColumn is not null` implies `TenantValue is not null` | Confirmed for **upsert** (all post-Part-A rows); pre-Part-A legacy rows remain a documented, accepted edge case (see Known limitations). **Corrected for delete** (critical-design-review round 1, finding 1): the authoritative Postgres row is already gone by delete-event-consume time, so this assumption cannot hold for `HandleDeleteAsync` by re-fetching Postgres — the design now sources the delete path's tenant value from `ev.PayloadJson` (the event's own pre-delete snapshot) instead; see "Call-site wiring" above. |
...
| Qdrant returns a distinguishable "not found" status for a search against a nonexistent collection, safely treatable as empty results (not confused with other errors) — **for the actual production credential (a scoped JWT), not just the admin key** | **Empirically tested directly, twice.** First pass: against the repo's own running `iverson-qdrant` container (v1.18.2) via `curl` with the admin key. **Corrected after critical-design-review round 1, finding 3** flagged that the first pass never exercised a scoped JWT (the running container has `jwt_rbac` off) — re-tested against a fresh, throwaway `qdrant/qdrant:v1.18.2` container with `jwt_rbac: true`, minting a real HS256 JWT and presenting it as the `api-key` header for both a read (search) and a write (upsert) against a fabricated collection name. | Confirmed for both credentials — HTTP 404 with an explicit `"doesn't exist"` error body either way; maps to gRPC `StatusCode.NotFound`. Also directly confirmed the core cross-tenant mechanism live in the same test: a JWT scoped to one real collection gets `200` reading/writing it, `403 Forbidden` against a second, different real collection. |
| A JWT's collection-access check is enforced against the name the *request* addresses (alias or physical), not silently resolved to whatever physical collection an alias currently points to — relevant because `MigrateCollectionAsync` renames the physical collection behind an alias on schema evolution | **Empirically tested** (added after critical-design-review round 1's span check flagged this as an uncovered dependency): created a physical collection, pointed an alias at it, then tested all combinations of {request via alias name, request via physical name} × {JWT scoped to alias name, JWT scoped to physical name} against a live `jwt_rbac`-enabled container | Confirmed — Qdrant checks the JWT's `access` claim against the exact name in the request path; a JWT scoped to the alias name is accepted when the request also addresses the alias name (`200`), and rejected (`403`) when either side names the physical name instead. Since this design always mints the JWT with, and addresses Qdrant by, the same tenant-qualified *logical* name, `MigrateCollectionAsync`'s alias-swap is fully transparent to this scheme — no gap. |
...
- The fail-closed sentinel path on **both** the read and write sides: a tenant-scoped call reached with a null tenant value mints a JWT scoped to the unreachable sentinel collection name, and the resulting Qdrant call fails rather than silently succeeding unfiltered. **Explicitly on the write side** (the path critical-design-review round 1 found broken): confirm `IntelligenceStoreConsumer` skips lazy-create when the tenant value is null, and that the subsequent upsert against the (still-nonexistent) sentinel collection actually fails — not that it silently succeeds because lazy-create ran anyway.
- `IntelligenceStoreConsumer`'s delete path correctly sources its tenant value from `ev.PayloadJson` (not a Postgres re-fetch, which would find the row already gone): a delete event for an existing tenant routes to that tenant's real collection and actually removes the point, not the sentinel.
- A read against a tenant with no collection yet returns an empty result (not an error) — proving the `NotFound`-as-empty handling, confirmed for the actual scoped-JWT credential production reads use, not just the admin key.
